...
cwd = os.getcwd()
rest_api_specs_path = r'/Users/travisprescott/repos/azure-rest-api-specs'
...
def generate(item, *, src, dest):
    ...
    print(f'== Generate code for {item.namespace}.{item.name} ==\n')
...
def main(argv):
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        for repo in repos_to_clone:
            ...
            out, err = run_command(f'git clone https://github.com/Azure/{repo}.git {tempdir}')
        ...
        for item in swagger_data:
            ...
            print(f'== Build package for {item.namespace}.{item.name} ==\n')
