...
public struct SealedConditionalSchema: ValueSchemaProtocol {
    /// the primitive type for the conditional
    public var conditionalType: PrimitiveSchemaProtocol

    /// the possible conditional values
    public var conditions: [ConditionalValue]

    /// the source value that drives the target value
    public var sourceValue: [ValueProtocol]

    // MARK: ValueSchemaProtocol
