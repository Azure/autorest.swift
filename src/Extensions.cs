...
namespace AutoRest.Swift
{
    public static class Extensions
    {
        ...
        public static string ToShortName(this string longName)
        {
            ...
            return CodeNamerSwift.Instance.GetVariableName(acronym);
        }
        ...
        {
            var r = body as CompositeTypeSwift;
        ...
        public static string GetEmptyCheck(this IModelType type, string valueReference, bool asEmpty = true)
        {
            ...
            {
                return (type as DictionaryTypeSwift).GetEmptyCheck(valueReference, asEmpty);
            }
            else if (type is EnumTypeSwift)
            {
                return (type as EnumTypeSwift).GetEmptyCheck(valueReference, asEmpty);
            }
            else
            {
        ...
        {
            if (type is DictionaryTypeSwift)
            {
                (type as DictionaryTypeSwift).AddImports(imports);
            }
            else if (type is PrimaryTypeSwift)
            {
        ...
        {            
            return (type is PrimaryType || type is SequenceType || type is DictionaryType || type is EnumType || 
                (type is CompositeTypeSwift && (type as CompositeTypeSwift).IsPolymorphicResponse()));
        }
            ...
            bool isCompositeProperties = false)
        {
            ...
            if (method != HttpMethod.Patch || !p.IsBodyParameter() || isCompositeProperties)
            {
                foreach (var prop in ((CompositeType)p.ModelType).Properties)
                {
                    ...
                    var map = prop.ModelType as DictionaryTypeSwift;
