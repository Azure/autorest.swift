...
public final class Duration {
    public let client: AutoRestDurationTestClient

    init(client: AutoRestDurationTestClient) {
        self.client = client
    }
    ...
    /// - Parameters:
    ///    - positiveDuration : duration body
    ///    - options: A list of options for the operation
    ///    - completionHandler: A completion handler that receives a status code on
    ///     success.
    public func put(
        positiveDuration: Iso8601Duration,
        withOptions options: PutPositiveDurationOptions? = nil,
        ...
        // Construct request
        guard let requestBody = try? JSONEncoder().encode(positiveDuration) else {
            client.options.logger.error("Failed to encode request body as json.")
            return
        }
        let urlTemplate = "/duration/positiveduration"
        guard let requestUrl = client.url(host: "{$host}", template: urlTemplate, params: params),
    ...
    ///    - completionHandler: A completion handler that receives a status code on
    ///     success.
    public func getPositiveDuration(
        withOptions options: GetPositiveDurationOptions? = nil,
        completionHandler: @escaping HTTPResultHandler<Iso8601Duration>
    ) {
        let dispatchQueue = options?.dispatchQueue ?? client.commonOptions.dispatchQueue ?? DispatchQueue.main
    ...
    public func getInvalid(
        ...
        // Construct request
        let urlTemplate = "/duration/invalid"
        guard let requestUrl = client.url(host: "{$host}", template: urlTemplate, params: params),
            let request = try? HTTPRequest(method: .get, url: requestUrl, headers: params.headers)
        else {
            client.options.logger.error("Failed to construct HTTP request.")
